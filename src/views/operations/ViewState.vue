        ...
        <b-row>
          <b-col>
            <base-block rounded>
              ...
              <b-table class="mb-2" @filtered="onFiltered" selectable show-empty striped hover bordered head-variant="light" :filter="filter" :items="centres" :fields="centreFields" :current-page="currentPage" :per-page="perPage" @row-clicked="viewZone" >
...
export default {
  mounted() {
    ...
    this.$store.dispatch('getStateDetails', {stateID: this.$store.getters.getStateParams})
    .then(response =>{
      this.centres = response.data.message.centres
      this.ccs = response.data.message.centres.length
  ...
  data(){
    return{
      ...
      ccs: 0,
      stateCount: 0,
      centreFields: [{key: 'cc_id', sortable: true, thStyle: 'width: 10%'}, {key: 'cc_name', sortable: true}, {key: 'actions', sortable: false, thStyle: 'width: 9px'}],
