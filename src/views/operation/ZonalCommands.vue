<template>
  <div>
    <!-- Hero -->
    <base-page-heading title="Zonal Commands" subtitle="View Details of All Zonal Commands Here">
      <template #extra>
      ...
      <b-row>
        <b-col>
          <base-block rounded title="All Zonal Commands" :subtitle="totalRows +' total zonal commands'" ref="manageZoneBlock" btn-option-fullscreen>
            ...
            <b-table class="mb-2" @filtered="onFiltered" selectable show-empty striped hover bordered head-variant="light" :filter="filter" :items="zones" :fields="zoneFields" :current-page="currentPage" :per-page="perPage" @row-clicked="viewZone" >
...
export default {
  ...
  methods: {
    ...
    viewZone(item){
      ...
      this.$store.commit('setZoneRouteParam', {zoneID})
